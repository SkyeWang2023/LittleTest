...
void test(const char *ifname){
  if(dhd_priv_ioctl(ifname,DHD_PRIV_EVENT_MSG_44_1,strlen(DHD_PRIV_EVENT_MSG_44_1))<0){} // not required
}
...
int testc1(int len, char* ent_model) {
  ...
  // nuwa是正确的
  strlen(useModel);  // not required //__DIFF__
}
...
void test_02(void) {
  ...
  if (cur_msg != 0) {
    ...
    cur_msg_len = wcslen(cur_msg);  // not required
  }
}
...
void test_02a(void) {
  ...
  temp_size = cur_msg_size / 2 + 1;
  temp = realloc(cur_msg, temp_size * sizeof(wchar_t));
  cur_msg_len = wcslen(temp);  // not required
}
...
void bc3e(void) {
  ...
  strlen(realloc(str, size * 2));  // not required
}
...
void bc3g(char *str) {
  ...
  strlen(myRealloc(str, size * 2));  // not required
}
