...
void test(void) {
  char c_str[3] = "abc";
  printf("%s\n", c_str);  // #defect#USE_UNTERMINATED_STRING
}
...
unsigned long long test2(const char *source) {
  ...
  if (source) {
    ...
    ret = strlen(c_str);  // #nodefect#USE_UNTERMINATED_STRING
  } else {
  }
  return ret;
}
...
void test01(void) {
  char c_str[3] = "abc";
  myPrintf(c_str);  // #defect#USE_UNTERMINATED_STRING
}
...
void test17(int s, char *p) {
  ...
  if (*p) {
    myStrlen(p - 3);  // #defect#USE_UNTERMINATED_STRING
  }
}
